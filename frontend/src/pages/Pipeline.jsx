...
const STAGE_ICONS = {
  ...
  expanding: "M13 10V3L4 14h7v7l9-11h-7z",
  queuing: "M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10",
  crawling: "M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15",
  deduplicating: "M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4",
  verifying: "M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z",
...
const STAGE_COLORS = {
  ...
  expanding: "text-blue-500",
  queuing: "text-amber-500",
  crawling: "text-indigo-500",
  deduplicating: "text-teal-500",
  verifying: "text-teal-600",
...
function LiveStats({ progress }) {
  if (!progress) return null;
  const stats = [
    { label: "Found", value: progress.companies_found, color: "text-indigo-600" },
    { label: "New", value: progress.companies_new, color: "text-green-600" },
    { label: "Merged", value: progress.companies_merged || 0, color: "text-teal-600" },
    { label: "Pages", value: progress.pages_crawled, color: "text-blue-600" },
    { label: "Emails", value: progress.emails_found, color: "text-blue-500" },
    { label: "Phones", value: progress.phones_found, color: "text-purple-600" },
    { label: "Websites", value: progress.websites_found, color: "text-teal-500" },
    { label: "Verified", value: progress.verified, color: "text-emerald-600" },
    { label: "Enriched", value: progress.enriched, color: "text-orange-600" },
    { label: "Scored", value: progress.scored, color: "text-pink-600" },
    { label: "Active", value: progress.active_jobs, color: "text-indigo-500" },
...
function LogFeed({ messages }) {
  ...
  return (
    <div ref={feedRef} className="space-y-1 max-h-[400px] overflow-y-auto p-3 bg-gray-50 dark:bg-dark-800 rounded-lg">
      {messages.map((msg, i) => (
...
export default function Pipeline() {
  ...
  // Auto-start if navigated from Search
  useEffect(() => {
    if (location.state?.autoStart && location.state?.runId) {
  ...
  const handlePreview = async () => {
    if (!query.trim()) return;
    try {
      const res = await expandQuery(query.trim(), maxQueries);
      setPreview(res.data);
      setShowPreview(true);
    } catch (err) {
  ...
  const statusColor = progress
    ? (progress.status === "completed" ? "border-green-500/30" :
       ...
       progress.status === "cancelled" ? "border-gray-500/30" :
    ...
    <div className="animate-fade-in">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">
          Discover companies across India from 5+ sources with automatic enrichment, verification, and scoring.
        </p>
      </div>
      ...
      <div className="glass-card p-5 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-4 pt-3 border-t border-gray-100 dark:border-dark-600">
          <div className="flex items-center gap-2">
            <label className="text-sm text-gray-500">Queries:</label>
            <select
              value={maxQueries}
          ...
          <div className="flex items-center gap-2">
            <label className="text-sm text-gray-500">Workers:</label>
            <select
              value={maxConcurrent}
          ...
          <label className="flex items-center gap-1.5 text-sm text-gray-500 cursor-pointer">
            <input type="checkbox" checked={skipVerify} onChange={(e) => setSkipVerify(e.target.checked)} disabled={running} className="rounded" />
            Skip Verify
          </label>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-gray-900 dark:text-white">
              ...
              <span className="ml-2 text-sm font-normal text-gray-500">
                (type: {preview.query_type}, locations: {preview.locations_covered})
              </span>
            </h3>
      ...
      {/* Live Pipeline View */}
      {progress && (
        <div className={`glass-card p-5 mb-6 border-2 ${statusColor}`}>
          {/* Status header */}
          <div className="flex items-center gap-3 mb-4">
            <div className={`p-2 rounded-lg ${
              ...
              progress.status === "cancelled" ? "bg-gray-100 dark:bg-gray-900/30" :
              "bg-indigo-100 dark:bg-indigo-900/30"
            }`}>
              <svg className={`w-5 h-5 ${STAGE_COLORS[progress.status] || "text-gray-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={STAGE_ICONS[progress.status] || STAGE_ICONS.planning} />
              </svg>
            </div>
            <div className="flex-1">
              <h3 className="font-semibold text-gray-900 dark:text-white capitalize">
                {progress.status === "completed" ? "Pipeline Complete" :
                 ...
                 progress.status === "cancelled" ? "Pipeline Cancelled" :
                 `Stage: ${progress.status}`}
              </h3>
              <p className="text-sm text-gray-500">
                Query: "{progress.query}"
                {progress.current_query && progress.status === "crawling" && (
                  <span> &middot; Current: "{progress.current_query}" on {progress.current_source}</span>
                )}
              </p>
            </div>
      ...
      {progress && progress.messages && (
        <div className="glass-card p-5">
          <h3 className="font-semibold text-gray-900 dark:text-white mb-3">
            Live Discovery Log
            {running && <span className="ml-2 inline-block w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>}
          </h3>
      ...
      {!progress && !running && (
        <div className="glass-card p-8 text-center">
          <svg className="w-16 h-16 text-gray-300 dark:text-dark-500 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <p className="text-sm text-gray-400 max-w-md mx-auto">
            Enter a search query above to start discovering companies.
            Each search is expanded into hundreds of variations across multiple sources.
          </p>
        </div>
      )}
    </div>
